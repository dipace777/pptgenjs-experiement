...
function fontShorthand(fontSizePx: number, spec: FontFaceSpec): string {
  ...
  parts.push(`${defaultFontFace(spec.family)}, Helvetica, sans-serif`);
  return parts.join(" ");
}
...
export function textElementOverflows(el: TextElement): boolean {
  const spec = textLayoutSpecFromElement(el);
  const measured = measureTextHeightInches(spec);
  if (measured == null) return false;
  return measured > elementBox(el).h + OVERFLOW_TOLERANCE_IN;
}
...
export function fitFontToBox(
  ...
  const spec = isTextElement(input) ? textLayoutSpecFromElement(input) : input;
  const targetHeight =
    heightInches ?? (isTextElement(input) ? elementBox(input).h : 0);
  if (typeof window === "undefined") return spec.fontSize;
  const target = Math.max(0.05, targetHeight - FIT_SAFETY_GAP_IN);
...
function measureBulletsHeightInches(
  ...
  const bulletGutterIn = el.marker === "none" ? 0 : (fontSizePt * 0.9) / 72;
...
function textLayoutSpecFromElement(el: TextElement): TextLayoutSpec {
  const box = elementBox(el);
  const font = elementFont(el);
  return {
    text: textContent(el),
...
function isTextElement(input: TextLayoutSpec | TextElement): input is TextElement {
  return "runs" in input;
}
