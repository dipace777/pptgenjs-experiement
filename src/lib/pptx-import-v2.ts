...
async function importFromZip(
  ...
  const slideIds = toArray(
    (presNode["p:sldIdLst"] as Record<string, unknown> | undefined)?.[
      "p:sldId"
    ],
...
async function spToElement(
  ...
  if (objectName === PPTY_IMAGE_PLACEHOLDER_TAG) {
    const nameAttr =
      typeof cNvPr?.["@_descr"] === "string"
        ? (cNvPr["@_descr"] as string)
        : undefined;
    return {
      type: "image",
...
function isRoundedCustomGeometry(
  ...
  const pathLst = custGeom?.["a:pathLst"] as
...
function extractShadow(
  ...
  const outer = effectLst?.["a:outerShdw"] as
    | Record<string, unknown>
    | undefined;
  ...
  return {
    ...
    blur: clamp(
      Number.isFinite(blurRaw) ? emuToIn(blurRaw) * scale : 0,
      0,
      100,
    ),
...
function orderedItemsFromNodes(
  nodes: OrderedXmlNode[],
): OrderedSlideTreeItem[] {
...
function findOrderedChildren(nodes: unknown, key: string): OrderedXmlNode[] {
